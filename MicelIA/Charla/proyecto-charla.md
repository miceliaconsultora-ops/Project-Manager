---
id: charla-siteba
name: Charlas de IA para SITEBA
category: Capacitación & IA
status: active
progress: 50
description: Brindar 4 charlas sobre IA en el mundo laboral bancario/financiero para la central gremial y gremio de trabajadores bancarios.
next_step_action: Dictar el Segundo Encuentro (14 de julio a las 17:30 hs) y procesar/subir sus materiales y grabación.
next_step_responsible: Diego R.
next_step_deadline: 2026-07-14
---

## PROYECTO CHARLA SITEBA

## INFO GENERAL
**Cliente:** SITEBA
**Objeto:** Brindar 4 charlas sobre IA en el mundo laboral bancario/financiero. 
**Contratante:** SITEBA
**Inicio:** 2026-06-23

## DESCRIPCION GENERAL
He sido contratado personalmente, para dar una suerte de curso o encuentros sucesivos, por parte de un gremio de trabajadores bancarios, sin embargo, la invitación será abierta a todo tipo de trabajadores de la central gremial a la que pertenece el gremio.

## PROXIMOS PASOS
- Dictar el Segundo Encuentro del ciclo ("Adopción Institucional: El rol del trabajador") el 14 de julio de 2026 a las 17:30 hs.
- Subir los nuevos materiales y grabación del Segundo Encuentro.
- Coordinar con el gremio los materiales y logística para los Encuentros 3 y 4.

## Registro de Avance
- 2026-07-14: Se subió el PDF "Para seguir aprendiendo" a la sección de materiales del Segundo Encuentro en index.html.
- 2026-07-11: Se agregaron nuevos materiales del Segundo Encuentro (Material de apoyo e Informe de geopolítica) y se sumó el Informe PyMEs Argentina 2026 al Primer Encuentro. Se reorganizaron los archivos físicos en subcarpetas (Encuentro 1 y Encuentro 2) y se actualizaron los enlaces en index.html, renombrando el acordeón a su título oficial "Adopción Institucional: El rol del trabajador".
- 2026-07-02: Se anunció el Segundo Encuentro para el 14 de julio a las 17:30 hs con su temario detallado en el Hero (elemento colapsable interactivo). Se sumó popup modal de bienvenida para inscripción y se retiraron accesos directos de Zoom por seguridad.
- 2026-06-30: Reorganización total de la sección de materiales en un acordeón interactivo por encuentros, subiendo el video del Primer Encuentro con carga diferida (lazy-load) y miniatura de YouTube, además de la presentación del mismo.
- 2026-06-23: Integración de accesos e ID de Zoom en Hero, sección de lugar y pantalla de éxito. Incorporación de dos nuevos materiales de apoyo descargables.
- 2026-06-21: Descarga y configuración local de 4 nuevos materiales (Estatutos, Decretos y simulador de prompts). Rediseño de sección materiales a una grilla de 5 tarjetas interactivas con links de respaldo a Drive.
- 2026-06-16: Descarga e integración de primer material de apoyo local (.docx) para Charla 1. Reemplazo de placeholder por card real con botón de descarga y prueba local.
- 2026-06-01: Modificado Formulario.
- 2026-05-31: Incorporar logos y modificar formulario. Esperar más información por parte del gremio.
- 2026-05-30: Primera versión de la web de inscripción creada. A la espera de información y contenidos por parte del gremio.

